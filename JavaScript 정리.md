...
- **`const`**:값을 재할당할 수 없는 상수 선언시 사용(값의 변경을 막아 안정성을 높이므로 기본적으로 권장됨)
```
let score = 90; // 재할당 가능
...
```
## 3. 반복문
>특정 조건이 만족될 때까지 코드 블록을 반복해서 실행함
- **`for`**:횟수 반복
- **`while`**:특정 조건이 참인 동안 계속 반복
```
for (let i = 0; i < 3; i++) {
  // i가 0, 1, 2일 때 실행 (3이 되면 3 < 3이 거짓이므로 종료)
...
- **배열**:순서가 있는 데이터의 목록.인덱스(0부터 시작)를 통해 접근함
...
- **배열과 객체의 활용**
    - **배열**은 순서가 중요할 때(예: 대기 순번, 점수 리스트) 주로 사용합니다.

    - **객체**는 하나의 대상에 대한 상세 정보(예: 회원 정보, 상품 상세 설명)를 담을 때 유리합니다.